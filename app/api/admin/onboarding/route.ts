...
export async function GET(request: NextRequest) {
  try {
    ...
    const usersWithOnboarding = profiles.map(
      (profile: {
        user_id: string;
        ...
        const currentStep =
          profile.onboarding_step || ONBOARDING_STEPS.EMAIL_VERIFICATION;
    ...
    const stats = {
      ...
      byStep: {
        emailVerification: usersWithOnboarding.filter(
          (u: { onboarding: { currentStep: number } }) =>
            u.onboarding.currentStep === ONBOARDING_STEPS.EMAIL_VERIFICATION,
        ).length,
        profileInfo: usersWithOnboarding.filter(
          (u: { onboarding: { currentStep: number } }) =>
            u.onboarding.currentStep === ONBOARDING_STEPS.PROFILE_INFO,
        ).length,
        accountType: usersWithOnboarding.filter(
          (u: { onboarding: { currentStep: number } }) =>
            u.onboarding.currentStep === ONBOARDING_STEPS.ACCOUNT_TYPE,
        ).length,
        companyInfo: usersWithOnboarding.filter(
          (u: { onboarding: { currentStep: number } }) =>
            u.onboarding.currentStep === ONBOARDING_STEPS.COMPANY_INFO,
        ).length,
        complete: usersWithOnboarding.filter(
          (u: { onboarding: { currentStep: number } }) =>
            u.onboarding.currentStep === ONBOARDING_STEPS.COMPLETE,
        ).length,
      },
      completed: usersWithOnboarding.filter(
        (u: { onboarding: { isComplete: boolean } }) => u.onboarding.isComplete,
      ).length,
      pendingApproval: usersWithOnboarding.filter(
        (u: { onboarding: { isComplete: boolean }; approvalStatus: string }) =>
          u.onboarding.isComplete && u.approvalStatus === "pending",
      ).length,

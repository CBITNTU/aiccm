...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("🗑️ Clearing all existing jobs and batches from processing queue...");
    ...
    if (deleteJobsError) {
      ...
      throw new Error(`Failed to clear existing jobs: ${deleteJobsError.message}`);
    } else {
      console.log(`✅ Cleared ${jobsDeleted || 'all'} existing jobs from processing queue`);
    }
    
    // Also clear old batch records
    const { error: deleteBatchesError, count: batchesDeleted } = await adminSupabase
    ...
    if (deleteBatchesError) {
      ...
      console.log(`✅ Cleared ${batchesDeleted || 'all'} existing batch records`);
    }
    ...
    if (tenderIds && Array.isArray(tenderIds) && tenderIds.length > 0) {
      ...
      tendersToProcess = ((allTenders || []) as unknown as { id: string }[]).map((t) => t.id);
    }
    ...
    const { batchId } = await enqueueBatch(jobs, "tender_ai_regeneration", user.id);

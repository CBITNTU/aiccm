...
export async function GET(request: NextRequest) {
  try {
    ...
    if (stats) {
      results.pendingJobs = stats.filter((j: any) => j.status === "pending").length;
      results.processingJobs = stats.filter((j: any) => j.status === "processing").length;
    }
    ...
    if (results.pendingJobs > 0) {
      ...
      try {
        ...
        if (!workerResponse.ok) {
          ...
          console.log(`✅ Triggered worker for ${results.pendingJobs} pending jobs`);
        }
      } catch (fetchError) {
    ...
    return NextResponse.json({
      ...
      ...results,
      message: results.stuckJobsReset > 0 

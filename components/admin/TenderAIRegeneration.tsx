...
export function TenderAIRegeneration() {
  ...
  const handleRegenerate = async () => {
    try {
      ...
      toast.success(`Queued ${data.jobCount} AI processing jobs for ${data.tenderCount} tenders`);
  ...
  return (
    <div className="space-y-4">
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Regenerate Tender AI Summaries & Taxonomies</DialogTitle>
          <DialogDescription>
            This will regenerate AI summaries and capability taxonomies for all tenders.
          ...
          {!batchId ? (
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                Click the button below to queue AI regeneration jobs for all tenders.
              </p>
              <Button
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{batch?.completedJobs || 0}</span>
                  </div>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">
                      {batch ? batch.totalJobs - batch.completedJobs - batch.failedJobs : 0}
                    </span>
                  </div>
              ...
              {batch && (
                <div className="text-sm">
                  <p>
                    <span className="font-medium">Total:</span> {batch.totalJobs} tenders
                  </p>
                  <p>
                    <span className="font-medium">Failed:</span> {batch.failedJobs}
                  </p>
                </div>
              )}
              ...
              {isComplete && (
                <div className="space-y-2">
                  <div className={`p-3 rounded-lg ${batch.status === "completed" ? "bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800" : "bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800"}`}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-medium ${batch.status === "completed" ? "text-green-900 dark:text-green-100" : "text-red-900 dark:text-red-100"}`}>
                        {batch.status === "completed" ? "Regeneration Complete!" : "Regeneration Failed"}
                      </span>
                    </div>
                  </div>
                  <Button
              ...
              {!isComplete && (
                <p className="text-xs text-muted-foreground text-center">
                  ...
                  Check back here to see progress.
                </p>
              )}
            </div>
          )}

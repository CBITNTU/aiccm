...
import { MapPin, Globe, Phone, Mail, Award, Building2, FileText, Wrench, Shield, Loader2 } from "lucide-react";
...
export function CompanyDetailModal({ company, open, onOpenChange }: CompanyDetailModalProps) {
  ...
  const isFullCompany = (comp: PublicCompany | Company | null): comp is Company => {
        ...
        <DialogHeader>
          ...
          <div className="mt-4">
            <h4 className="text-sm font-semibold flex items-center gap-2 mb-3">
              ...
              Key Capabilities
              {analysis && <Badge variant="outline" className="text-xs">AI Analyzed</Badge>}
            </h4>
            ...
            {!analysis && company.key_capabilities && (
              <p className="text-xs text-muted-foreground mt-2">
                <span className="text-yellow-600">💡</span> Click "Analyze Company" to get AI-powered capability insights
              </p>
            )}
          </div>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {company.safety_rating && (
                <div className="border border-border rounded-lg p-4 bg-card hover:bg-accent/5 transition-colors">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground block">Safety Rating</label>
                    <p className="text-sm text-green-700 dark:text-green-300 font-medium leading-relaxed break-words">{company.safety_rating}</p>
                  </div>
                </div>
              )}
              ...
              {company.digital_maturity && (
                <div className="border border-border rounded-lg p-4 bg-card hover:bg-accent/5 transition-colors">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground block">Digital Maturity</label>
                    <p className="text-sm text-blue-700 dark:text-blue-300 font-medium leading-relaxed break-words">{company.digital_maturity}</p>
                  </div>
                </div>
              )}
              ...
              {company.market_position && (
                <div className="border border-border rounded-lg p-4 bg-card hover:bg-accent/5 transition-colors">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground block">Market Position</label>
                    <p className="text-sm text-muted-foreground leading-relaxed break-words">{company.market_position}</p>
                  </div>
                </div>
              )}
                ...
                <div className="border border-border rounded-lg p-4 bg-card hover:bg-accent/5 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <label className="text-sm font-medium text-foreground block">Company Status</label>
                      <p className="text-xs text-muted-foreground mt-0.5">Current operational status</p>
                    </div>
              ...
              {isOwner && (
                ...
                >
                  {loadingAnalysis ? (
                    ...
                    analysis ? 'Re-analyze Performance' : 'Analyze Performance'
                  )}
                </Button>
              )}
            </div>
            ...
            {analysis ? (
                  ...
                  {radarData.map((item, index) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-muted/50 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-xs min-w-[2rem] text-right">{item.A}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="h-[280px] flex items-center justify-center text-muted-foreground border border-dashed border-border rounded-lg bg-muted/30">
                <div className="text-center">
                  ...
                  {isOwner ? (
                    <p className="text-sm">Click "Analyze Performance" to generate your company's benchmark scores</p>
                  ) : (
                    <p className="text-sm">Performance analysis not available for this company</p>
                  )}
                </div>
              </div>
            )}

...
const UKCompaniesMap: React.FC<UKCompaniesMapProps> = ({
  companyId
}) => {
  ...
  const fetchCompanies = async () => {
    try {
      let query = supabase.from('companies').select('id, company_name, address, postcode, status').eq('status', 'active').not('postcode', 'is', null);
  ...
  const geocodeAddress = async (address: string, postcode: string): Promise<{
    lat: number;
    lon: number;
  } | null> => {
    try {
  ...
  useEffect(() => {
    if (companies.length > 0 && markers.length === 0 && !geocoding) {
      geocodeCompanies();
    }
  }, [companies, markers.length, geocoding]);
  if (loading) {
    return <div className="w-full h-[400px] flex items-center justify-center bg-gradient-to-br from-muted/30 to-muted/10 rounded-2xl border border-border/20">
  ...
  return <div className="relative w-full h-[400px] rounded-2xl overflow-hidden shadow-xl border border-border/20 bg-card">
      {/* Map Header */}
      ...
      <div className="uk-map relative w-full h-full flex items-center justify-start p-8 pt-24">
        <div className="relative w-full h-full max-w-md">
          {/* UK Map SVG */}
          <img src={ukMapSvg} alt="UK Map" className="w-full h-full object-contain opacity-80" />
          
          {/* Company Pins */}
          <svg className="absolute inset-0 w-full h-full" viewBox={`0 0 ${SVG_VIEWBOX.width} ${SVG_VIEWBOX.height}`}>
            {markers.map(marker => <g key={marker.company.id}>
                {/* Pin */}
                <circle cx={marker.position.x} cy={marker.position.y} r="4" className="fill-red-500 cursor-pointer transition-all hover:r-6" style={{
              filter: hoveredMarker === marker.company.id ? 'drop-shadow(0 0 8px rgb(239 68 68))' : 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))'
            }} onMouseEnter={() => setHoveredMarker(marker.company.id)} onMouseLeave={() => setHoveredMarker(null)} />
                
                {/* Tooltip */}
                {hoveredMarker === marker.company.id && <g>
                    <foreignObject x={marker.position.x + 5} y={marker.position.y - 15} width="120" height="60">
      ...
      {!companyId && <div className="absolute right-0 top-0 bottom-0 w-80 bg-background/95 backdrop-blur-sm border-l border-border overflow-y-auto">
          <div className="p-6">
            ...
            <div className="space-y-2">
              {markers.map(marker => <div key={marker.company.id} className={`p-3 rounded-lg border transition-all cursor-pointer ${hoveredMarker === marker.company.id ? 'border-primary bg-primary/5' : 'border-border/50 hover:border-border'}`} onMouseEnter={() => setHoveredMarker(marker.company.id)} onMouseLeave={() => setHoveredMarker(null)}>

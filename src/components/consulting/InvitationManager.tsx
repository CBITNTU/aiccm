...
export function InvitationManager({ members, onSendInvitations, projectTitle }: InvitationManagerProps) {
  const [selectedPartners, setSelectedPartners] = useState<string[]>([]);

  // Filter out lead and already accepted members
  const invitableMembers = members.filter(m => m.role === 'invited' || m.role === 'member');
      ...
      <CardContent>
            ...
            {invitableMembers.map((member) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <div className="font-medium">{member.companies?.company_name}</div>
        ...
        {invitableMembers.length > 0 && (
          <div className="mt-4 p-4 bg-muted rounded-lg">
            ...
            <div className="text-sm text-muted-foreground">
              <p><strong>Subject:</strong> Invitation to collaborate on {projectTitle}</p>
              <p className="mt-2">
                <strong>Message:</strong> You've been invited to join a consulting team for the tender "{projectTitle}". 
                Please review the project details and accept the invitation to collaborate.
              </p>
            </div>
          </div>
        )}

...
import { Trash2, RefreshCw, Users, Loader2, Crown, Plus, Target, Lightbulb } from "lucide-react";
...
export function TeamBuilder({ members, onRemoveMember, onRunGroupAnalysis, onAddCompany, analyzing, teamAnalysis }: TeamBuilderProps) {
          ...
          <div className="mt-6 pt-6 border-t space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className="flex items-baseline gap-2 mt-2">
                    <span className="text-3xl font-bold text-green-600">{teamAnalysis.coveragePercentage}%</span>
                    <Badge variant={teamAnalysis.coveragePercentage >= 90 ? "default" : "secondary"}>
                      {teamAnalysis.coveragePercentage >= 90 ? "Excellent" : teamAnalysis.coveragePercentage >= 70 ? "Good" : "Needs Work"}
                    </Badge>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className="flex items-baseline gap-2 mt-2">
                    <span className="text-3xl font-bold text-green-600">{teamAnalysis.readinessScore}%</span>
                    <Badge variant={teamAnalysis.readinessScore >= 85 ? "default" : "secondary"}>
                      {teamAnalysis.readinessScore >= 85 ? "Ready to Bid" : "Almost Ready"}
                    </Badge>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {teamAnalysis.missingCompetencies && teamAnalysis.missingCompetencies.length > 0 && (
                  <div>
                    <Label className="font-semibold text-orange-500">Remaining Gaps</Label>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {teamAnalysis.missingCompetencies.map((comp, idx) => (
                        <Badge key={idx} variant="outline" className="bg-orange-500/10 border-orange-500/20">
                          {comp}
                        </Badge>
                      ))}
                    </div>
            ...
            {teamAnalysis.risks && teamAnalysis.risks.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-base text-red-500">Team Risks</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2">
